...
wpd.graphicsWidget = (function() {
    ...
    // OpenDigitizer: allow controllers to programmatically route keyboard events
    // to the active canvas tool. Needed because clicking a sidebar control
    // (e.g. the keyboard-mode toggle) blurs the canvas, which would otherwise
    // stop arrow/A/Z/C keystrokes from reaching the tool.
    function setCanvasFocus(focused) {
        isCanvasInFocus = (focused === undefined) ? true : !!focused;
    }
