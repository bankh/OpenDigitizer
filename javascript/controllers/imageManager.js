...
wpd.imageManager = (function() {
    ...
    // OpenDigitizer: load an explicit list of files (e.g. every image picked
    // from a folder) as a multi-file session, then show the first one.
    function loadFiles(files) {
        if (files && files.length > 0) {
            _newLoad = true;
            initializeFileManager(files);
            wpd.appData.setPageManager(null);
