...
wpd.algoManager = (function() {
    ...
    function updateAlgoList() {
        ...
        // OpenDigitizer: append pluggable detectors registered via the
        // detector registry that support the current axes type.
        if (wpd.detectorRegistry) {
            const extraDetectors = wpd.detectorRegistry.getForAxes(axes);
            for (let d of extraDetectors) {
                innerHTML += '<option value="' + d.id + '">' + d.name + '</option>';
            }
        }
        ...
        if (autoDetector.algorithm != null) {
            ...
            } else if (autoDetector.algorithm.detectorId) {
                // OpenDigitizer: pluggable registry detector
                $algoOptions.value = autoDetector.algorithm.detectorId;
            }
    ...
    function applyAlgoSelection() {
        ...
        if (wpd.detectorRegistry && wpd.detectorRegistry.has(selectedValue)) {
            // OpenDigitizer: pluggable detector from the registry
            autoDetector.algorithm = wpd.detectorRegistry.createAlgo(selectedValue);
        } else if (selectedValue === 'averagingWindow') {
    ...
    function run() {
        ...
        // OpenDigitizer: treat the template matcher AND any detector that flags
        // itself async (e.g. WASM workers, remote HTTP detectors) the same way.
        const isAsync = (algo instanceof wpd.TemplateMatcherAlgo) || algo.isAsync === true;
        if (isAsync) {
...
wpd.dataMask = (function() {
    ...
    function markBox() {
        // OpenDigitizer: Box with "Polygon" checkbox -> click-to-click filled polygon
        const cb = document.getElementById('box-polygon-toggle');
        if (cb && cb.checked) {
            wpd.graphicsWidget.setTool(new wpd.PolygonMaskTool({
                fill: true,
                buttonId: 'box-mask',
                containerId: 'mask-box-container'
            }));
        } else {
            wpd.graphicsWidget.setTool(new wpd.BoxMaskTool());
        }
    }

    function markPen() {
        // OpenDigitizer: Pen with "Polyline" checkbox -> click-to-click straight lines
        const cb = document.getElementById('pen-polyline-toggle');
        if (cb && cb.checked) {
            wpd.graphicsWidget.setTool(new wpd.PolygonMaskTool({
                fill: false,
                buttonId: 'pen-mask',
                containerId: 'mask-paint-container'
            }));
        } else {
            wpd.graphicsWidget.setTool(new wpd.PenMaskTool());
        }
    }
