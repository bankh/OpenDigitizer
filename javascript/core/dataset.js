...
wpd.Dataset = class {
    ...
    // OpenDigitizer: remove a single pixel from the current selection (for
    // additive/toggle selection with Shift/Ctrl-click).
    unselectPixel(index) {
        const i = this._selections.indexOf(index);
        if (i >= 0) {
    ...
    // OpenDigitizer: snapshot of the point-level state for undo/redo.
    // Captures everything that point add/move/delete operations can mutate
    // (data points, point-group tuples, selections, per-pixel metadata bookkeeping)
    // but NOT structural definitions like group names, which point ops don't touch.
    getDataPointsState() {
        return {
            ...
            pixelMetadataKeys: this._pixelMetadataKeys.slice()
        };
    }

    // OpenDigitizer: restore a snapshot produced by getDataPointsState().
    applyDataPointsState(state) {
        if (state == null) {
            return;
        }
        this._dataPoints = JSON.parse(JSON.stringify(state.dataPoints));
        this._tuples = JSON.parse(JSON.stringify(state.tuples));
        this._selections = state.selections.slice();
        this._pixelMetadataCount = state.pixelMetadataCount;
        this._pixelMetadataKeys = state.pixelMetadataKeys.slice();
    }
